...
const enum Size {
  ...
  Medium,
  Large
} // with const it is more optimise code
...
function incomeTaxCalculate(income: number, year?: number): number {
  // use old js || operator
  if ((year || 2020) < 2022) return income * 1.2;
...
function TaxCalculate(income: number, year = 2020): number {
  // use predefine value
  if (year < 2022) return income * 1.2;
